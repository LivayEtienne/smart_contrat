...
// POST /api/depots - Creer un depot (investisseur connecte)
...
// GET /api/depots - Mes depots (investisseur connecte)
...
// PUT /api/depots/:depot_id/valider - Valider un depot (admin)
...
// PUT /api/depots/:depot_id/refuser - Refuser un depot (admin)
const refuser = async (req, res) => {
  try {
    ...
    if (!motif) {
      return res.status(400).json({ message: "Un motif de refus est obligatoire" });
    }
...
// GET /api/depots/admin/tous - Tous les depots (admin)
...
// PUT /api/depots/wallet - Mettre a jour le wallet address
